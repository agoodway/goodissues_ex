...
FruitFly's telemetry system currently uses a custom reporter format with a Phoenix-specific data model. This limits it to Elixir/Phoenix apps and couples clients to a proprietary protocol. By adopting OpenTelemetry Protocol (OTLP) as the ingestion format, FruitFly can collect traces and metrics from any language or framework that has an OTel SDK — which is effectively all of them. Users point their standard OTLP exporter at FruitFly and get full observability correlated with their error tracking.
...
- Storage behaviour (`FF.Otel.Storage`) with Postgres v1 implementation, designed for swappable backends (ClickHouse, BigQuery)
- Per-project TTL-based data pruning via Oban cron job
- Drop existing `telemetry_spans` table and `/api/v1/events` endpoint
- FruitflyReporter exception data correlates with OTel traces via `trace_id`
...
- `telemetry-events-api`: Drop `/api/v1/events` endpoint and FruitflyReporter custom format
- `telemetry-spans`: Drop `telemetry_spans` table and `FF.Telemetry` context
...
- **Schemas**: New `FF.Otel.Span`, `FF.Otel.Metric` Ecto schemas; updated `FF.Tracking.Project` with new fields
- **Contexts**: New `FF.Otel` context with storage behaviour; remove `FF.Telemetry` context
...
- **Workers**: New `FF.Otel.Workers.RetentionPruner` Oban cron job
