...
- [ ] 2.5 Create `GI.Otel.Span` Ecto schema with type specs
- [ ] 2.6 Create `GI.Otel.Metric` Ecto schema with type specs
- [ ] 2.7 Update `GI.Tracking.Project` schema with `otel_service_name` and `retention_days` fields; update create/update changesets to cast and validate them (otel_service_name optional, retention_days >= 1)
...
- [ ] 4.1 Create `GI.Otel` context module with `ingest_traces/2` and `ingest_metrics/2` functions — accepts account + decoded proto, resolves projects via `otel_service_name`, delegates to storage
- [ ] 4.2 Add `resolve_project_by_service_name/2` to `GI.Tracking` — looks up project by `otel_service_name` within account
...
- [ ] 6.1 Create `GI.Otel.Workers.RetentionPruner` Oban cron worker — iterates projects with retention_days set, calls storage.prune for each
- [ ] 6.2 Add cron config: `{"0 3 * * *", GI.Otel.Workers.RetentionPruner}` in Oban config
