...
- [x] 1.1 Add `checks_topic/1` function to `FF.Monitoring` that returns `"checks:project:<project_id>"` topic string
- [x] 1.2 Add broadcast helpers to `FF.Monitoring` for `:check_created` and `:check_updated` flat check payloads, plus a `:check_deleted` payload containing the deleted check id
...
- [x] 2.1 Add `count_checks_by_status/2` function to `FF.Monitoring` that returns `%{up: n, down: n, unknown: n, paused: n}` for a project scoped to an account
...
- [x] 4.1 Create `FFWeb.Dashboard.CheckLive.Index` LiveView module — mount loads project from URL param, verifies account ownership, subscribes to PubSub topic
...
- [x] 5.1 Create `FFWeb.Dashboard.CheckLive.New` LiveView module — mount with permission check, initialize changeset
...
- [x] 6.1 Create `FFWeb.Dashboard.CheckLive.Show` LiveView module — mount loads check scoped to project and account, subscribes to PubSub for live status updates
