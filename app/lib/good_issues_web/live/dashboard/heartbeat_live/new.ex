defmodule GIWeb.Dashboard.HeartbeatLive.New do
  @moduledoc """
  Dashboard view for creating a new heartbeat monitor.
  ...
  def mount(%{"project_id" => project_id}, _session, socket) do
    ...
    if not Scope.can_manage_account?(socket.assigns.current_scope) do
      {:ok,
       socket
       |> put_flash(:error, "You don't have permission to create heartbeats.")
       |> push_navigate(to: ~p"/dashboard/#{account.slug}/projects/#{project_id}/heartbeats")}
    else
      case Tracking.get_project(account, project_id) do
        ...
        project ->
          changeset = Monitoring.change_heartbeat(%Heartbeat{})

          {:ok,
           socket
           |> assign(:page_title, "New Heartbeat")
  ...
  def handle_event("save", %{"heartbeat" => heartbeat_params}, socket) do
    if Scope.can_manage_account?(socket.assigns.current_scope) do
      ...
      heartbeat_params = Map.put(heartbeat_params, "project_id", project.id)

      case Monitoring.create_heartbeat(account, user, heartbeat_params) do
        {:ok, heartbeat} ->
          {:noreply,
           socket
           |> put_flash(:info, "Heartbeat created successfully.")
           |> push_navigate(
             to: ~p"/dashboard/#{account.slug}/projects/#{project.id}/heartbeats/#{heartbeat.id}"
           )}
      ...
      <div class="h-full flex flex-col">
        ...
        <div class="px-4 sm:px-6 py-4 sm:py-5 border-b border-base-300/50 bg-base-100">
          ...
          <div class="flex items-center gap-3 sm:gap-4">
            ...
            <div>
              <h1 class="text-base sm:text-lg font-semibold text-base-content">New Heartbeat</h1>
              <div class="font-mono text-[11px] sm:text-xs text-muted mt-0.5">
                Add a heartbeat monitor for {@project.name}
              </div>
            </div>
          </div>
        </div>
