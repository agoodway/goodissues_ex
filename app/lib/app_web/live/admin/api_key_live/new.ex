defmodule FFWeb.Admin.ApiKeyLive.New do
  use FFWeb, :live_view
  ...
  def mount(_params, _session, socket) do
    account_users = Accounts.list_all_account_users()

    account_user_options =
      Enum.map(account_users, fn au ->
        {"#{au.user.email} (#{au.account.name})", au.id}
      end)

    {:ok,
     socket
     |> assign(:page_title, "New API Key")
     |> assign(:account_user_options, account_user_options)
  ...
  def handle_event("save", %{"api_key" => api_key_params}, socket) do
    account_user_id = api_key_params["account_user_id"]

    if account_user_id && account_user_id != "" do
      account_user = Accounts.get_account_user!(account_user_id)
      ...
      changeset =
        %ApiKey{}
        |> Accounts.change_api_key(api_key_params)
        |> Ecto.Changeset.add_error(:account_user_id, "is required")
        |> Map.put(:action, :validate)
  ...
  def render(assigns) do
    ...
    <FFWeb.Layouts.admin flash={@flash} current_scope={@current_scope} page_title={@page_title}>
      <.header>
        New API Key
        <:subtitle>Create a new API key for any account member</:subtitle>
      </.header>
        ...
        <div class="mt-6">
          <div class="alert alert-warning mb-6">
            ...
            <div>
              ...
              <p>This token will only be shown once and cannot be retrieved later. Copy and store it securely.</p>
            </div>
          </div>
          ...
          <div class="card bg-base-200">
            <div class="card-body">
              ...
              <div class="card-actions justify-end mt-4">
                <.link navigate={~p"/admin/api-keys/#{@created_api_key.id}"} class="btn btn-primary">
      ...
      <% else %>
        <div class="mt-6 max-w-xl">
          <.form for={@form} id="api-key-form" phx-change="validate" phx-submit="save">
            ...
            <.input
              field={@form[:account_user_id]}
              type="select"
              label="Owner (User + Account)"
              options={[{"Select owner...", ""} | @account_user_options]}
              required
            />
